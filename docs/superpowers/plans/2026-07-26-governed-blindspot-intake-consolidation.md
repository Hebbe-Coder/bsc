...
| P04 | Authorized REST lifecycle, single `dbos_intake` MCP facade, existing MCP HTTP catalog wiring, TypeScript client contract, and persisted-state Control Center panel. | `app/api/dbos_api.py`, `app/api/mcp_http.py`, `app/mcp/dbos_tools.py`, `app/mcp/server.py`, `src/api/dbosApi.ts`, `src/components/dbos/BlindspotIntakePanel.tsx`; REST/MCP and frontend tests below. | Disable feature; existing API/MCP transports and permissions retain their prior behavior. |
| P05 | Focused behavior coverage, production type/build checks, and browser interaction evidence. | 52 backend/API/MCP tests plus 12 frontend component tests, recorded below. | No schema rollback or destructive cleanup is needed; use the feature flag. |
...
# 52 passed, 1 Starlette/httpx deprecation warning

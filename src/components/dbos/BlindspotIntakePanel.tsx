...
import { ArrowRight, FileOutput, RefreshCw, RotateCcw, Sparkles } from 'lucide-react';
...
export function BlindspotIntakePanel({ projectId, sessionId = '', disabled = false, onMissionConverted }: Props) {
  ...
  useEffect(() => {
    if (!sessionId || intake?.artifact_id === sessionId) return;
    let active = true;
    void fetchDbosIntake(projectId, sessionId)
      .then((restored) => { if (active) setIntake(restored); })
      .catch((reason) => { if (active) setError(reason instanceof Error ? reason.message : 'Unable to read Intake state.'); });
    return () => { active = false; };
  }, [intake?.artifact_id, projectId, sessionId]);
  ...
  const loadQuestion = async (sessionId: string) => {
    const result = await nextDbosIntakeQuestion(projectId, sessionId);

...
export function PersonalGrowthCockpit({ projectId, onClose, runtimeAccessKey = '', onConfigureAccess, initialMissionId = '', onStartMission, onOpenOutputReview }: Props) {
  ...
  const importCopilotTranscript = async () => {
  ...
  const openCopilotDelivery = async () => {
    ...
    try {
      ...
      setCopilotDeliveryNotice('Copilot delivery command was invoked in Obsidian. Detected files remain pending D-layer sync and review.');

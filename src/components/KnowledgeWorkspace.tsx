...
function pluginRuntimeDetail(plugin: KnowledgeWorkspaceData['plugins']['plugins'][number]): string {
  if (plugin.id === 'copilot' && plugin.model_readiness?.state === 'unavailable') return 'Enable Copilot Plus or configure the selected provider in Obsidian before opening delivery.';
  if (plugin.id === 'copilot' && plugin.model_readiness?.state === 'unverified') return 'Copilot provider access is stored outside the readable settings file; complete one real response in Obsidian before treating delivery as available.';

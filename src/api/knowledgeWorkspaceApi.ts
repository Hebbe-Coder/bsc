...
export const fetchKnowledgeCopilotCommands = (projectId: string) => request<{ state: 'available'; commands: Array<{ key: string; name: string; available: boolean }> }>(`/knowledge/workspaces/${encodeURIComponent(projectId)}/copilot/commands`);
export const invokeKnowledgeCopilotCommand = (projectId: string, commandKey: string) => post<{ run_id: string; state: 'invoked'; command: { key: string; name: string } }>(`/knowledge/workspaces/${encodeURIComponent(projectId)}/copilot/commands/${encodeURIComponent(commandKey)}`, {});
